<template>
  <div class="app-container" :class="loading === false && peptideList.length >=9 ? 'bg-image' : ''">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="TCR氨基酸序列" label-width="120px" prop="hla">
        <el-input
            v-model="queryParams.hla"
            placeholder="请输入TCR氨基酸序列"
            clearable
    ...
    <el-table v-loading="loading" style="opacity:0.8;width:100%" :data="peptideList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column fixed label="TCR氨基酸序列" width="400px" align="center" prop="hla" />
    ...
    <el-dialog :title="title" :visible.sync="upload" width="500px" append-to-body>
      <el-form ref="uploadForm" :model="uploadForm" label-width="80px" >
        <el-form-item label="上传文件">
          <my-file-upload-tcr ref="myFileUpload" @processing="processing" @processSuccess="getList"
...
<script>
import { listPeptide, getPeptide, delPeptide, addPeptide, updatePeptide, getData, tcrChange } from "@/api/medical/peptide";
import MyFileUploadTcr from "@/components/MyFileUploadTcr"
import ThermodynamicChart from "@/views/dashboard/ThermodynamicChart.vue";
import {download} from "@/utils/request";
export default {
  name: "Peptide",
  dicts: ['sys_model', 'sys_change'],
  components: {ThermodynamicChart, MyFileUploadTcr },
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    /** 查询亲和度列表 */ getList() {
      ...
      this.$set(this.queryParams, "isTcr" , 1);
    ...
    handleSeeChange(row) {
      this.$router.push({
        path: '/visitor/tcrMutation?id=' + row.id,
      })
    },
    /** 提交按钮 */
    submitForm() {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除该TCR氨基酸序列').then(function() {
    ...
    handleDownload() {
      download("/medical/peptide/downloadTcpTemplate/", "","TCR_Template.csv");
    },
    handleClose(done) {
      this.drawer = false;
    },
