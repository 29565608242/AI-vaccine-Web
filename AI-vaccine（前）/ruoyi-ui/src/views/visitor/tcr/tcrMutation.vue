<template>
  <div class="app-container" :class="loading === false && mutationList.length >=9 ? 'bg-image' : ''">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label-width="120px" label="TCR氨基酸序列" prop="hla">
        <el-input
...
export default {
  ...
  methods: {
    ...
    handleDownloadGraph(row) {
      ...
      download("/medical/mutation/downTcrGraph/" + id +"/" +mutationId, "","突变肽结构图.png");
    },
    handleDownloadFile(row) {
      var id = this.$route.query.id;
      var mutationId = row.id
      download("/medical/mutation/downTcrFile/" + id +"/" +mutationId, "","MutPeptideStructure.pdb");
    },
    // 表单重置
    reset() {
      this.form = {
